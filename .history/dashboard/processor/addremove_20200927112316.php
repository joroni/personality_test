...
if($request_type == 1){
    ...
    if($result->num_rows > 0){
       while($row = $result->fetch_assoc()) {
            $data[] = array("user_earth"=>$row['user_earth'],"user_air"=>$row['user_air'],"user_water"=>$row['user_water'], "user_fire"=>$row['user_fire'],"group_id"=>$row['group_id'],"first_name"=>$row['first_name'],
...
if($request_type == 3){
    ...
    if($result->num_rows == 0){
       // INSERT INTO `users` (`id`, `user_name`, `user_password`, `email`, `first_name`, `last_name`, `group_id`, `date_joined`, `is_active`, `acl`) VALUES (NULL, 'jay', '', 'jay@mail.com', 'jay', 'yo', '2', '2020-09-25 15:07:19', '1', '2');
        ...
        $insertSQL = "INSERT INTO users (email, first_name, last_name, group_id, acl) VALUES (?, ?, ?,?,?)";
        $stmt = $con->prepare($insertSQL);
        $stmt->bind_param("sssss",$email,$first_name,$last_name,$group_id,$acl);
        $stmt->execute();

        $lastinsert_id = $stmt->insert_id;
        if($lastinsert_id > 0){
            $return_arr[] = array("id"=>$lastinsert_id,"email"=>$email,"first_name"=>$first_name,"last_name"=>$last_name,"group_id"=>$group_id, "acl"=>$acl);
        }
        $stmt->close();
