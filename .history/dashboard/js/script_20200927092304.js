...
/*   for (var i = 0; i < team.length; i++) {
 ...
 // $scope.getAUserScore(user_id);
};

  $scope.getAUserScore = function (team) {
 //   index = this.$index;
   /*  self.select = function(index) {
      $scope.series.push(index);
  } */
  /* $scope.series =[];
  for (var i = 0; i < team.length; i++) {
 ...
 */
  ...
  $scope.getUser = function (team) {
    $scope.user_id = team.user_id;
    $http({
      method: 'post',
      url: 'processor/addremove.php',
      data: {
