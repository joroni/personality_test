 ...
 $scope.data = [];
  ...
  $scope.onSendClick = function(event)
  {
    ...
    $http({
      ...
      $scope.link = "http://localhost/personality_test/frontend/?email="+ $scope.userSendData.email+"&user_id="+$scope.userSendData.id+"&group_id="+$scope.userSendData.group_id;

   

   /*  for (var i = 0; i < $scope.userSendData.length; i++) { 
      $scope.userlinkparam.join({
        'id': $scope.userSendData[i].id,  
        'email': $scope.userSendData[i].email,
        'group_id': $scope.userSendData[i].group_id,  
        });      
    }
    */
     });
