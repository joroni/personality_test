...
/* 
  ...
  $scope.getAUserScore = function () {
    console.log('getAUserScore');
    $http({
      method: 'post',
      url: 'processor/addremove.php',
      data: {
      ...
      console.log(usersData);
     // $scope.allUsers = usersData;
      $scope.auserdata = usersData;
      
      $scope.labels = ["Earth", "Air", "Water", "Fire"];
      //$scope.series = ['Susan', 'Joroni', 'Jane'];
      $scope.series = [];
      for (var i = 0; i < $scope.auserdata.length; i++) {
      
        $scope.series.push(
          $scope.auserdata[i].first_name,
        );
      }
      console.log($scope.series );
      ...
      for (var i = 0; i < $scope.auserdata.length; i++) {
        ...
        $scope.data.push([
          $scope.auserdata[i].user_earth,
          $scope.auserdata[i].user_air,
          $scope.auserdata[i].user_water,
          $scope.auserdata[i].user_fire
        ]);
      }
      console.log($scope.data );
      $scope.onClick = function (points, evt) {
