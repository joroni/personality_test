...
//  $scope.userdata.user_id  ={};
  $scope.getUserInfo = function () {
    for (let i = 0; i <= userdata.length; i++) {
      ...
      console.log('$scope.userdata.user_id', $scope.userdata.user_id);
...
$scope.isLast= function() {
  ...
  /*   for (let i = 0; i < lol.choice_id.length; i++) {
    ...
    sessionStorage.setItem('myresult', JSON.stringify($scope.dataresult));
  /*   } */
  $scope.addScore();
    });
   }
  ...
  $scope.addScore = function () {
   //let newuserdata = {};
    ...
    for (let i = 0; i < myscoring.length; i++) {
      $scope.myscores.push({
        'user_earth': myscoring[0],
        'user_air': myscoring[1],
        'user_water': myscoring[2],
        'user_fire': myscoring[3],
      });
    } 
