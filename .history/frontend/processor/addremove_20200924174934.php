...
if($request_type == 8){
    ...
    if($result->num_rows == 0){
        ...
        // Insert
        //INSERT INTO `answers` (`id`, `user_id`, `question_id`, `choice_id`, `group_id`, `time_answered`) VALUES (NULL, '12', '16', '1234', '1', current_timestamp());
        $insertSQL = "INSERT INTO user_scores (user_id, user_earth, user_water, user_air, user_fire, group_id) VALUES (?, ?, ?,?,?,?)";
        $stmt = $con->prepare($insertSQL);
        $stmt->bind_param("ssssss",$user_id,$user_earth,$user_water,$user_air,$user_fire,$group_id);
        $stmt->execute();

        $lastinsert_id = $stmt->insert_id;
        if($lastinsert_id > 0){
            $return_arr[] = array("id"=>$lastinsert_id,"user_id"=>$user_id,user_earth"=>$user_earth,"user_water"=>$user_water,"user_air"=>$user_air,"user_fire"=>$user_fire,"group_id"=>$group_id);
        }
        $stmt->close();
