...
if($request_type == 5){
   ...
   // $email = 'jraymund.niconi@gmail.com';
    ...
    $stmt = $con->prepare("SELECT * FROM user_data WHERE user_email = ?");
    ...
    if($result->num_rows > 0){
       while($row = $result->fetch_assoc()) {
            $data[] = array("user_id"=>$row['user_id'],"first_name"=>$row['first_name'],"user_email"=>$row['user_email']); 
        }
    }
...
if($request_type == 6){
    ...
    if($result->num_rows > 0){
       while($row = $result->fetch_assoc()) {
            $data[] = array("user_id"=>$row['user_id'],"first_name"=>$row['first_name'],"last_name"=>$row['last_name'],"user_name"=>$row['user_name'],"user_email"=>$row['user_email']); 
        }
    }
