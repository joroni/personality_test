...
app.controller("quizCtrl", function ($scope, $http) {
  ...
  /*
  Bold	3	Accurate	1	Patient	4	Enthusiastic
Facts	3	Logic	1	Feelings	2	Possibilities
Aggressive	3	Thorough	1	Caring	2	Sociable
Goals	3	Quality	4	Agreement	1	Ideas
Competitive	3	Analytical	1	Local	2	Optimistic
Use it	4	Check it	1	Share it	2	Possibilities
  */
  // Get all records
  /*$http({
    method: 'post',
...
*/
  ...
  // $scope.answers = [];
  // Add new record
  $scope.add = function (anwers) {
    $scope.len = $scope.answers.length;
    $http({
      method: 'post',
      url: 'processor/addremove.php',
      data: {
        'user_id': $scope.user_id[this.$index],
        'question_id': $scope.question_id[this.$index],
        'choice_id': $scope.choice_id[this.$index],
        'request_type': 2,
        'len': len
      },
