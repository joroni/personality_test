...
app.controller("quizCtrl", function ($scope) {
  ...
  $scope.selectColumn = function (selectedhead) {
   ...
   // console.log('selected column', $scope.selectedColumn);
    ...
    newpriority = $scope.data.priorities;
    for (let i = 0; i < newpriority.length; i++) { // Add options dynamically from OS Element
      this.newpriority.push({
        label: newpriority[i].value,
        value: newpriority[i].label
      });
    }
    $scope.runningData.push(newpriority);
