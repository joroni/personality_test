...
$scope.submitAnswer = function(myAnswer){
  ...
  $scope.add = function(){
    ...
    var len = $scope.user_question_answer.length;
    $http({
    method: 'post',
    url: 'processor/addremove.php',
    data: {user_id:$scope.user_id,username:$scope.username,question_id:$scope.question_id,choice_id:$scope.choice_id,request_type:2,len:len},
    }).then(function successCallback(response) {
        if(response.data.length > 0)
            $scope.user_question_answer.push(response.data[0]);
        else
