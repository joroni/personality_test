...
/*
   let newpriorities = $scope.priorities;
    console.log('newpriorities',newpriorities);
    for (let i = 0; i < newpriorities.length; i++) { // Add options dynamically from OS Element
      newpriorities.push({
        label: newpriorities[i].value,
        value: newpriorities[i].label
      });
    }
    for (let i = 0; i < $scope.runningData.length; i++) {
      console.log($scope.runningData.length);
    $scope.runningData.push({
        label: $scope.runningData[i].value,
        value: $scope.priorities[i].label
      })
    };
*/

  $scope.runningData.push($scope.data[selectedhead]);
