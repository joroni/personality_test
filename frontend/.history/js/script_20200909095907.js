...
*/
  ...
  $scope.selectColumn = function (selectedhead) {
    ...
    for (let i = 0; i < data.length; i++) {
      var obj =  $scope.runningData.push(selectedhead);
      for (let j = 0; j < data[i].length; j++) {
        obj.selectedhead = data[i].selectedhead;
      }
    }
     /* obj.qID = data[i].selectedhead;
      obj.opID = data[i].selectedhead;
      data[i].selectedhead.forEach(function(a,c){
      Object.keys(data[i].selectedhead[c]).forEach(function(a,b){
        obj[a] = Object.values(data[i].selectedhead[c])[b]
      })*/
     // })
     // MainInfo.push(obj);
    //}
...
var MainInfo=[];
            for (i = 0; i < data.length; i++) {
                ...
                data[i].priorities.forEach(function(a,c){
                Object.keys(data[i].priorities[c]).forEach(function(a,b){
                  obj[a] = Object.values(data[i].priorities[c])[b]
                })
                })
                MainInfo.push(obj);
            }
