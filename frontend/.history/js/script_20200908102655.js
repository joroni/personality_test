...
app.controller("quizCtrl", function ($scope) {
  ...
  $scope.initData = function () {
   ...
   $scope.questionData = $scope.source;
    for (let i = 0; i < $scope.questionData.length; i++) {
      console.log('srcData', $scope.questionData[i]);
      $scope.datavalue.push($scope.questionData[i]);
      $scope.priorities = $scope.questionData[i].priorities;
      // console.log('data', $scope.datavalue[i]);
      
      console.log('question', $scope.priorities);
    }
  ...
  $scope.selectColumn = function (selectedhead) {
    ...
    $scope.question = srcData[selectedhead].question;
    console.log('selected question',  $scope.question);
   // $scope.priorities = $scope.question
    /*let srcData = $scope.source;
    for (let i = 0; i < srcData.length; i++) {
      $scope.question = $scope.datavalue[i].question;
      $scope.datavalue.push(srcData[i]);
      for (let j = 0; j < srcData[i].options.length; j++) {
        $scope.datavalue.push(srcData[i].options[j]);
        $scope.priorities = $scope.datavalue[j].priorities;
        // console.log('data', $scope.datavalue[i]);
...
*/
    $scope.runningData.push($scope.question);
