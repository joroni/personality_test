...
if($request_type == 1){
    ...
    if($result->num_rows > 0){
       while($row = $result->fetch_assoc()) {
            $data[] = array("question_id"=>$row['question_id'],"question"=>$row['question'],"type"=>$row['type'],"choice_id"=>$row['choice_id']); 
        }
    }
...
if($request_type == 2){
    ...
    $stmt = $con->prepare("SELECT * FROM answers WHERE question_id=?");
